...
function getCharset (req) {
  ...
  const charset = contentType.parse(header).parameters.charset
...
function normalizeOptions (options, defaultType) {
  ...
  const inflate = !options || options.inflate !== false
  const limit = !options || typeof options.limit === 'undefined' || options.limit === null
    ? 102400 // 100kb default
    : bytes.parse(options.limit)
  const type = (options && options.type) || defaultType
  const verify = (options && options.verify) || false
  const defaultCharset = (options && options.defaultCharset) || 'utf-8'
