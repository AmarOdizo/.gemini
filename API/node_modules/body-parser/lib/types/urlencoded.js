...
function urlencoded (options) {
  ...
  const readOptions = Object.assign({}, normalizedOptions, {
    // assert charset
...
function createQueryParser (options) {
  ...
  let parameterLimit = (options && options.parameterLimit !== undefined)
    ? options.parameterLimit
    : 1000
  const charsetSentinel = options && options.charsetSentinel
  const interpretNumericEntities = options && options.interpretNumericEntities
  const depth = extended ? (options && options.depth !== undefined ? options.depth : 32) : 0
