...
function flattenArrRt(arr) {
  var res = [];
  for (var i = 0; i < arr.length; i++) {
    if (Array.isArray(arr[i])) res = res.concat(flattenArrRt(arr[i]));
    else res.push(arr[i]);
  }
  return res;
}
const flatten = { call: function(arr) { return Array.prototype.flat ? arr.flat(Infinity) : flattenArrRt(arr); } }
