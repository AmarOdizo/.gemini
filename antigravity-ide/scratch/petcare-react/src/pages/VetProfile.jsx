...
const VetProfile = () => {
  ...
  useEffect(() => {
    ...
    const fetchVet = async () => {
      try {
        const res = await fetch(`http://localhost:5000/api/vets/${vetId}`);
        if (res.ok) {
    ...
    if (storedUser) {
      ...
      fetch(`http://localhost:5000/api/pets?ownerId=${parsedUser.id || parsedUser._id}`, {
        headers: {
      ...
      // Fetch favorites
      fetch(`http://localhost:5000/api/favorites?ownerId=${parsedUser.id || parsedUser._id}`)
        .then(res => res.json())
        .then(data => {
          if (data.success && data.data) {
