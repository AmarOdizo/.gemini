...
const VetSidebar = () => {
  ...
  const handleFeedbackSubmit = async (e) => {
    ...
    try {
      ...
      const currentUser = storedUser ? JSON.parse(storedUser) : { _id: 'guest', name: 'Guest Vet' };
      
      const payload = {
        vetId: currentUser._id || currentUser.id || 'vet-feedback',
        vetName: currentUser.name || 'Vet User',
        ownerId: 'platform-feedback',
        ownerName: 'Vet Platform Feedback',
    ...
    <>
      <nav className="bg-surface-container-low border-r border-outline-variant/30 hidden md:flex flex-col h-screen w-[280px] p-6 gap-4 fixed left-0 top-0 z-40 no-print">
        ...
        <div className="flex flex-col gap-2 mt-auto pt-4 border-t border-outline-variant/20">
          <button onClick={() => setShowFeedbackModal(true)} className="flex items-center gap-3 px-4 py-2 text-on-surface-variant hover:bg-surface-container transition-all rounded-xl text-xs font-semibold cursor-pointer w-full text-left">
